    ...
    <section id="about" data-testid="about-section" className="py-24 sm:py-32 lg:py-40 bg-[#F3F0EA]">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 grid lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-7 lg:col-start-6">
          <p className="text-lg sm:text-xl text-[#0A192F]/80 leading-relaxed">
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-8 border-t border-[#0A192F]/15 pt-10">
            {[
              { k: "Years in market", v: "6+" },
              { k: "Concert tours", v: "40+" },
              { k: "Avg ROAS", v: "5×" },
              { k: "Client referrals", v: "92%" },
            ].map((s) => (
              <div key={s.k} data-testid={`about-stat-${s.k}`}>
                <p className="font-display text-3xl sm:text-4xl text-[#D4AF37]">{s.v}</p>
                <p className="text-xs uppercase tracking-[0.18em] text-[#0A192F]/60 mt-2">{s.k}</p>
              </div>
            ))}
          </div>
        </div>
